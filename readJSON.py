...
def readJSONSGSConfiguration(addPath):
    if os.path.isfile(addPath + "SGSConfiguration.JSON"):
        ...
        config.SGSConfigurationJSON = {"SGSConfigVersion": "001", "Valves": []}
...
def readJSON(addPath):
    ...
    if os.path.isfile(addPath + "SGS.JSON"):
        ...
        with open(addPath + "SGS.JSON") as json_file:
            ...
            config.INTERVAL_CAM_PICS__SECONDS = getJSONValue(
                "INTERVAL_CAM_PICS__SECONDS"
            )
            config.STATIONKEY = getJSONValue("STATIONKEY")
            config.WeatherUnderground_Present = getJSONValue(
                "WeatherUnderground_Present"
            )
            config.WeatherUnderground_StationID = getJSONValue(
                "WeatherUnderground_StationID"
            )
            config.WeatherUnderground_StationKey = getJSONValue(
...
def JSONsetDefaults():
    ...
    config.dataDefaults[
        "INTERVAL_CAM_PICS__SECONDS"
    ] = config.INTERVAL_CAM_PICS__SECONDS
    ...
    config.dataDefaults[
        "WeatherUnderground_StationID"
    ] = config.WeatherUnderground_StationID
    config.dataDefaults[
        "WeatherUnderground_StationKey"
    ] = config.WeatherUnderground_StationKey
