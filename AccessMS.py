...
def readMSSensor(adsdevice, gdedevice, channel):
    ...
    if config.SWDEBUG:
        print(
            "Channel #%d -%s- Moisture_Raw =%0.2f, %0.2f"
            % (channel, "Cap:", Moisture_Raw, Moisture_Humidity)
        )
    ...
    if config.SWDEBUG:
        print(
            "Channel #%d Pre Limit Moisture_Humidity=%0.2f"
            % (channel, Moisture_Humidity)
        )
...
def scaleMoistureCapacitance1(Moisture_Raw, PlantNumber):
    ...
    if Moisture_Raw < sensorCal[1]:
        ...
        Moisture_Humidity = ((sensorCal[0] - Moisture_Raw) * 100.0) / (
            sensorCal[0] - sensorCal[1]
        )
