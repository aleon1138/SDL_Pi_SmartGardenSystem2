...
class SGSConfigure(App):
    def __init__(self, *args):
        ...
        super(SGSConfigure, self).__init__(
    ...
    def setDefaults(self):
        ...
        self.dataDefaults[
            "INTERVAL_CAM_PICS__SECONDS"
        ] = self.INTERVAL_CAM_PICS__SECONDS
        self.dataDefaults["STATIONKEY"] = self.STATIONKEY
        self.dataDefaults[
            "WeatherUnderground_Present"
        ] = self.WeatherUnderground_Present
        self.dataDefaults[
            "WeatherUnderground_StationID"
        ] = self.WeatherUnderground_StationID
        self.dataDefaults[
            "WeatherUnderground_StationKey"
        ] = self.WeatherUnderground_StationKey
    ...
    def readJSONSGSConfiguration(self):
        if os.path.isfile("SGSConfiguration.JSON"):
            ...
            self.SGSConfigurationJSON = {"SGSConfigVersion": "001", "Valves": []}
    ...
    def readJSON(self):
        ...
        if os.path.isfile("SGS.JSON"):
            ...
            with open("SGS.JSON") as json_file:
                ...
                self.BMP280_Altitude_Meters = self.getJSONValue(
                    "BMP280_Altitude_Meters"
                )
                ...
                self.INTERVAL_CAM_PICS__SECONDS = self.getJSONValue(
                    "INTERVAL_CAM_PICS__SECONDS"
                )
                self.STATIONKEY = self.getJSONValue("STATIONKEY")
                self.WeatherUnderground_Present = self.getJSONValue(
                    "WeatherUnderground_Present"
                )
                self.WeatherUnderground_StationID = self.getJSONValue(
                    "WeatherUnderground_StationID"
                )
                self.WeatherUnderground_StationKey = self.getJSONValue(
                    "WeatherUnderground_StationKey"
                )
                self.USEBLYNK = self.getJSONValue("USEBLYNK")
                self.BLYNK_AUTH = self.getJSONValue("BLYNK_AUTH")
                self.AS3935_Lightning_Config = self.getJSONValue(
                    "AS3935_Lightning_Config"
                )
    ...
    def saveJSON(self):
        ...
        data[
            "INTERVAL_CAM_PICS__SECONDS"
        ] = self.F_INTERVAL_CAM_PICS__SECONDS.get_value()
        data["STATIONKEY"] = self.F_STATIONKEY.get_value()
        data[
            "WeatherUnderground_Present"
        ] = self.F_WeatherUnderground_Present.get_value()
        data[
            "WeatherUnderground_StationID"
        ] = self.F_WeatherUnderground_StationID.get_value()
        data[
            "WeatherUnderground_StationKey"
        ] = self.F_WeatherUnderground_StationKey.get_value()
    ...
    def buildScreen0(self):
        ...
        if len(self.WirelessDeviceJSON) == 0:
            ...
            scanForHardware = gui.Button(
                "Scan For SGS Hardware",
                style="height: 30px; width:200px;  margin: 10px;  top:5px",
            )
            scanForHardware.onclick.do(self.ScanForHardware)
            statusbox.append(scanForHardware, "scanForHardware")

        else:
            RescanForHardware = gui.Button(
        ...
        self.Display_WEXT.set_text(
            "Found Wireless Extenders: " + str(len(self.WirelessDeviceJSON))
        )
        ...
        self.ValveBlock = gui.Container(
            width=1000, height=700, layout_orientation=gui.Container.LAYOUT_HORIZONTAL
        )
        ...
        self.listView1 = gui.ListView.new_from_list(
            items, width=400, height=25 * len(valves), margin="10px"
        )
    ...
    def buildAValve(self, DeviceID, name, myUnit):
        myValve = gui.Container(
            width=300, height=700, layout_orientation=gui.Container.LAYOUT_VERTICAL
        )
        ...
        if len(str(ext[1])) < 4:
            for i in range(1, 5):
                self.valvelist = self.valvelist + (
                    str(ext[0]) + "/" + str(ext[1]) + "/Valve " + str(i),
                )
        ...
        self.DisplaySelect = gui.Label(
            "Valve Select", width=100, height=15, margin="5px"
        )
        self.dropDownValve = gui.DropDown.new_from_list(
            self.valvelist, width=200, height=20, margin="10px"
        )

        self.dropDownValve.onchange.do(self.drop_down_valve_changed)
        self.dropDownValve.select_by_value("None")
        self.DisplayControl = gui.Label(
            "Valve Control", width=100, height=15, margin="5px"
        )

        self.dropDownMSSensor = gui.DropDown.new_from_list(
            self.ValveControlItems, width=200, height=20, margin="10px"
        )

        self.dropDownMSSensor.select_by_value("Off")
        self.dropDownMSSensor.onchange.do(self.drop_down_MS_changed)

        self.DisplayMS = gui.Label(
            "Moisture Sensor Threshold Percent", width=300, height=15, margin="10px"
        )
        self.DisplayST_MS = gui.TextInput(width=100, height=15, style="margin:10px")
        self.DisplayDOW = gui.Label(
            "Day of Week Filter", width=200, height=15, margin="10px"
        )
        self.Display_Su = gui.CheckBoxLabel(
            "Su", False, height=30, style="margin:5px; background: LightGray "
        )
        self.Display_Mo = gui.CheckBoxLabel(
            "Mo", False, height=30, style="margin:5px; background: LightGray "
        )
        self.Display_Tu = gui.CheckBoxLabel(
            "Tu", False, height=30, style="margin:5px; background: LightGray "
        )
        self.Display_We = gui.CheckBoxLabel(
            "We", False, height=30, style="margin:5px; background: LightGray "
        )
        self.Display_Th = gui.CheckBoxLabel(
            "Th", False, height=30, style="margin:5px; background: LightGray "
        )
        self.Display_Fr = gui.CheckBoxLabel(
            "Fr", False, height=30, style="margin:5px; background: LightGray "
        )
        self.Display_Sa = gui.CheckBoxLabel(
            "Sa", False, height=30, style="margin:5px; background: LightGray "
        )

        self.DisplayTimed = gui.Label(
            "Timer Selection", width=200, height=15, margin="10px"
        )
        self.dropDownTimed = gui.DropDown.new_from_list(
            self.TimedItems, width=200, height=20, margin="10px"
        )
        ...
        self.DisplayOTS = gui.Label(
            "On Time Length in Seconds", width=200, height=15, margin="10px"
        )

        self.DisplayOTS_TB = gui.TextInput(width=100, height=15, style="margin:10px")
        self.DisplaySG_CB = gui.CheckBoxLabel(
            "Display Graph",
            False,
            height=30,
    ...
    def buildScreen05(self):
        ...
        screen1header = gui.Label(
            "Valve Configuration Report", style="margin:10px; background: LightGray"
        )
        ...
        self.table = gui.Table.new_from_list(
            myArray, width=600, height=400, margin="10px"
        )
    ...
    def buildScreen06(self):
        ...
        screen1header = gui.Label(
            "Change Extender Names", style="margin:10px; background: LightGray"
        )
        vbox.append(screen1header)
        ...
        self.listView2 = gui.ListView.new_from_list(
            items, width=400, height=25 * len(items), margin="10px"
        )
    ...
    def open_input_dialog(self, widget, myUnit):
        ...
        self.inputDialog = gui.InputDialog(
            "Changing " + myUnit, "New name?", initial_value=name, width=200
        )
    ...
    def buildScreen1(self):
        ...
        screen1header = gui.Label(
            "Debug / MySQL /MW Tab", style="margin:10px; background: LightGray"
        )
        vbox.append(screen1header)

        # debug config

        debugheader = gui.Label(
            "Debug Configuration",
            style="position:absolute; left:5px; top:30px; " + self.headerstyle,
        )
        vbox.append(debugheader, "debugheader")
        self.F_SWDEBUG = gui.CheckBoxLabel(
            "enable SW Debugging",
            self.SWDEBUG,
            height=30,
            style="margin:5px; background: LightGray ",
        ...
        plabel = gui.Label(
            "SGS Manual and Tank Control",
            style="position:absolute; left:5px; top:40px;" + self.labelstyle,
        )

        self.F_manual_water = gui.CheckBoxLabel(
        ...
        self.F_English_Metric = gui.CheckBoxLabel(
    ...
    def buildScreen2(self):
        ...
        vbox = VBox(
            width=1000, height=510, style="background: LightGray; border: 5px solid red"
        )
        ...
        MTheader = gui.Label(
            "Mail and Text Notification Configuration",
            style="position:absolute; left:5px; top:30px;" + self.headerstyle,
        )
        ...
        self.F_enableText = gui.CheckBoxLabel(
        ...
        self.F_textnotifyAddress = gui.TextInput(
            width=300, height=30, style="margin:5px"
        )
    ...
    def buildScreen3(self):
        ...
        # screen 1
        screen1header = gui.Label(
            "Pixel / NeoPixel / SolarMAX Configuration Tab", style="margin:10px"
        )
        vbox.append(screen1header)

        PNheader = gui.Label(
            "Pixel/NeoPixel LED Support",
            style="position:absolute; left:5px; top:30px;" + self.headerstyle,
        )
        vbox.append(PNheader, "PNheader")

        self.F_enablePixel = gui.CheckBoxLabel(
            "Enable Pixel/NeoPixel",
            self.enablePixel,
            height=30,
            style="margin:5px; background: LightGray ",
        )
        ...
        P1Nheader = gui.Label(
            "Solar Max Configuration",
            style="position:absolute; left:5px; top:30px;" + self.headerstyle,
        )
        vbox.append(P1Nheader, "P1Nheader")

        self.F_SolarMAX_Present = gui.CheckBoxLabel(
            "SolarMAX Present",
            self.SolarMAX_Present,
            height=30,
            style="margin:5px; background: LightGray ",
        )
        ...
        self.F_BMP280_Altitude_Meters = gui.TextInput(
            width=300, height=30, style="margin:5px"
        )
    ...
    def buildScreen4(self):
        ...
        # screen
        screen1header = gui.Label(
            "Weather / WeatherSTEM / WeatherUnderGround Configuration Tab",
            style="margin:10px",
        )
        vbox.append(screen1header)

        P3_1Nheader = gui.Label(
            "Use Weather",
            style="position:absolute; left:5px; top:30px;" + self.headerstyle,
        )
        vbox.append(P3_1Nheader, "P3_1Nheader")

        self.F_weather = gui.CheckBoxLabel(
            "Use Weather",
            self.weather,
            height=30,
            style="margin:5px; background: LightGray ",
        ...
        self.F_USEWEATHERSTEM = gui.CheckBoxLabel(
            "Enable WeatherSTEM",
            self.USEWEATHERSTEM,
            height=30,
            style="margin:5px; background: LightGray ",
        ...
        p5label = gui.Label(
            "Interval between pictures (seconds)",
            style="position:absolute; left:5px; top:40px;" + self.labelstyle,
        )
        vbox.append(p5label, "p5label")

        self.F_INTERVAL_CAM_PICS__SECONDS = gui.TextInput(
            width=300, height=30, style="margin:5px"
        )
        self.F_INTERVAL_CAM_PICS__SECONDS.set_value(
            str(self.INTERVAL_CAM_PICS__SECONDS)
        )
        vbox.append(self.F_INTERVAL_CAM_PICS__SECONDS, "INTERVAL_CAM_PICS__SECONDS")

        p6label = gui.Label(
            "GardenCam Station Key",
            style="position:absolute; left:5px; top:40px;" + self.labelstyle,
        )
        ...
        P4Nheader = gui.Label(
            "WeatherUnderGround Configuration",
            style="position:absolute; left:5px; top:30px;" + self.headerstyle,
        )
        vbox.append(P4Nheader, "P4Nheader")

        self.F_WeatherUnderground_Present = gui.CheckBoxLabel(
        ...
        vbox.append(
            self.F_WeatherUnderground_Present, "self.F_WeatherUnderground_Present"
        )
        ...
        self.F_WeatherUnderground_StationID = gui.TextInput(
            width=300, height=30, style="margin:5px"
        )
        ...
        self.F_WeatherUnderground_StationKey = gui.TextInput(
            width=300, height=30, style="margin:5px"
        )
        self.F_WeatherUnderground_StationKey.set_value(
            self.WeatherUnderground_StationKey
        )
        vbox.append(
            self.F_WeatherUnderground_StationKey, "WeatherUnderground_StationKey"
        )
    ...
    def buildScreen5(self):
        ...
        # screen 1
        screen1header = gui.Label(
            "Blynk / ThunderBoard AS3935 Tab", style="margin:10px"
        )
        vbox.append(screen1header)
        ...
        self.F_USEBLYNK = gui.CheckBoxLabel(
            "Enable Blynk",
            self.USEBLYNK,
            height=30,
            style="margin:5px; background: LightGray ",
        ...
        P2Nheader = gui.Label(
            ...
            style="position:absolute; left:5px; top:30px;" + self.labelstyle,
        )
        vbox.append(P2Nheader, "P2Nheader")
        ...
        self.F_AS3935_Lightning_Config = gui.TextInput(
            width=300, height=30, style="margin:5px"
        )
    ...
    def buildScreen7(self):
        ...
        vbox.append(P5Nheader, "P5Nheader")
        self.F_Camera_Night_Enable = gui.CheckBoxLabel(
            "Night Vision Enable",
            self.Camera_Night_Enable,
            height=30,
            style="margin:5px; background: LightGray ",
        )
        ...
        self.F_REST_Enable = gui.CheckBoxLabel(
            "REST Enable",
            self.REST_Enable,
            height=30,
            style="margin:5px; background: LightGray ",
        ...
        self.F_MQTT_Enable = gui.CheckBoxLabel(
            "MQTT Enable",
            self.MQTT_Enable,
            height=30,
            style="margin:5px; background: LightGray ",
        ...
        self.F_MQTT_Port_Number = gui.TextInput(
            width=200, height=30, style="margin:5px"
        )
        ...
        self.F_MQTT_Send_Seconds = gui.TextInput(
            width=200, height=30, style="margin:5px"
        )
    ...
    def main(self):
        ...
        header = gui.Label(
            "Smart Garden System Configuration Tool V004",
        ...
        saveandreload = gui.Button(
            "Save and Reload SGS",
            style="position:absolute; left:675px; height: 30px; width:100px;  margin: 10px;  top:5px",
        )
        saveandreload.onclick.do(self.onSaveAndReloadSGS)
        exit = gui.Button(
            "Save and Exit",
            style="position:absolute; left:500px; height: 30px; width:100px;  margin: 10px;  top:95px",
        )
        ...
        self.labelstyle = (
            "font-family:monospace; font-size:15px; margin:5px; background:LightGray"
        )
    ...
    def ScanForHardware(self, widget, name="", surname=""):
        ...
        if self.SFHWorking == False:
            ...
            SFHThread = threading.Thread(
                target=self.ScanForHardwareWorker, args=("name",)
            )
    ...
    def ScanForHardwareWorker(self, name):
        ...
        for ip in ipaddress.IPv4Network(CIDR):
            ...
            self.Display_WEXT.set_text(
                "Found Wireless Extenders: " + str(len(returnJSON))
            )
            JSON = ""
