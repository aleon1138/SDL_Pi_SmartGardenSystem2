...
def valveCheck():
    ...
    for single in myValves:
        ...
        if scanForResources.isDeviceActive(single["id"]) == True:
            ...
            if (single["Control"] == "Timed") and (single["DOWCoverage"] != "NNNNNNN"):
                ...
                if stateValveCheck(single["id"], single["ValveNumber"]):
                    ...
                    if NextTime <= nowTime:
                        ...
                        if dayDelay > 0:
                            nowTime = datetime.datetime.now() + datetime.timedelta(
                                days=dayDelay
                            )
                            nowTime = nowTime.replace(
                                hour=int(myTempTime[0]),
                        ...
                        stateValveUpdateTime(
                            single["id"], single["ValveNumber"], NextTime
                        )
                ...
                if state.nextMoistureSensorActivate < datetime.datetime.now():
                    ...
                    if float(myMoistureReading) > 5.0:
                        ...
                        if myThreshold > float(myMoistureReading):
                            ...
                            if config.SWDEBUG:
                                print(
                                    "Valve Turned On by MS Sensor#%s/%s/%s for %s Seconds "
                                    % (MSNumber, Name, myID, single["OnTimeInSeconds"])
                                )
                            pclogging.valvelog(
                                single["id"],
                                single["ValveNumber"],
            ...
            if config.SWDEBUG:
                print(
                    "Inactive Wireless Device %s / Valve %s "
                    % (str(single["id"]), str(single["ValveNumber"]))
                )
    ...
    while state.nextMoistureSensorActivate < myNow:
        state.nextMoistureSensorActivate = (
            state.nextMoistureSensorActivate + datetime.timedelta(minutes=15)
        )
        if config.SWDEBUG:
...
def calculateFirstTime(single):
    ...
    myStartTime = nowTime.replace(
        hour=int(myTempTime[0]), minute=int(myTempTime[1]), second=0, microsecond=0
    )
...
def stateValveCheck(myID, myValveNumber):
    ...
    for vState in state.valveStatus:
        if str(vState["id"]).replace(" ", "") == str(myID).replace(" ", ""):
            if str(vState["ValveNumber"]).replace(" ", "") == str(
                myValveNumber
            ).replace(" ", ""):
                return True
