    ...
    if config.enable_MySQL_Logging == True:
        ...
        if level >= LOWESTDEBUG:
            try:
                ...
                con = mdb.connect(
                    "localhost", "root", config.MySQL_Password, "SmartGardenSystem"
                )
                cur = con.cursor()
                # print "before query"
                query = (
                    "INSERT INTO SystemLog(TimeStamp, Level, SystemText ) VALUES(LOCALTIMESTAMP(), %i, '%s')"
                    % (level, message)
                )
                # print("query=%s" % query)
                cur.execute(query)
...
def sensorlog(DeviceID, SensorNumber, SensorValue, SensorType, TimeRead):
    if config.enable_MySQL_Logging == True:
        ...
        try:
            ...
            cur = con.cursor()
            query = (
                "INSERT INTO Sensors(DeviceID, SensorNumber, SensorValue, SensorType, TimeRead ) VALUES('%s', '%s', %f, '%s', '%s')"
                % (DeviceID, SensorNumber, float(SensorValue), SensorType, TimeRead)
            )
            # print("query=", query)
            cur.execute(query)
            con.commit()
...
def getValveState(id):
    if config.enable_MySQL_Logging == True:
        ...
        try:
            ...
            cur = con.cursor()
            query = (
                "SELECT * From ValveRecord WHERE DeviceID = '%s' ORDER BY ID DESC LIMIT 1"
                % id
            )
            # print("query=", query)
            cur.execute(query)
...
def valvelog(DeviceID, ValveNumber, State, Source, ValveType, Seconds):
    if config.enable_MySQL_Logging == True:
        ...
        try:
            ...
            cur = con.cursor()
            query = (
                "INSERT INTO ValveChanges(DeviceID, ValveNumber, State, Source, ValveType, SecondsOn ) VALUES('%s', '%s', %d, '%s', '%s',%d)"
                % (DeviceID, ValveNumber, int(State), Source, ValveType, int(Seconds))
            )
            # print("query=", query)
            cur.execute(query)
            con.commit()
...
def writeMQTTValveChangeRecord(MQTTJSON):
    ...
    if config.enable_MySQL_Logging == True:
        ...
        try:
            ...
            cur = con.cursor()
            query = "INSERT INTO ValveRecord(DeviceID, State) VALUES('%s', '%s')" % (
                MQTTJSON["id"],
                MQTTJSON["valvestate"],
            )
            # print("query=", query)
...
def writeWeatherRecord():
    ...
    if config.enable_MySQL_Logging == True:
        ...
        try:
            ...
            query = (
                "SELECT id, AQI, TimeStamp FROM WeatherData WHERE (TimeStamp > '%s') ORDER BY TimeStamp "
                % (before)
            )
            ...
            values = (
                "%6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f"
                % (
                    state.OutdoorTemperature,
                    state.OutdoorHumidity,
                    state.IndoorTemperature,
                    state.IndoorHumidity,
                    state.TotalRain,
                    state.SunlightVisible,
                    state.SunlightUVIndex,
                    state.WindSpeed,
                    state.WindGust,
                    state.WindDirection,
                    state.BarometricPressure,
                    state.BarometricPressureSeaLevel,
                    state.BarometricTemperature,
                    state.AQI,
                    state.Hour24_AQI,
                )
            )
            ...
            if len(state.IndoorTH) > 0:
                ...
                for singleChannel in state.IndoorTH:
                    values = '%d, %d, %6.2f, %6.2f, "%s", "%s"' % (
                        singleChannel["deviceID"],
                        singleChannel["channelID"],
                        singleChannel["temperature"],
                        singleChannel["humidity"],
                        singleChannel["batteryOK"],
                        singleChannel["time"],
                    )
