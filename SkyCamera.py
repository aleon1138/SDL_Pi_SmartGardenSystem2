...
def takeSkyPicture():
    ...
    try:
        ...
        myText = "SmartGardenSystem2 V%s %s Wind Speed: %s Wind Gust: %s Temp: %s " % (
            config.SGSVERSION,
...
def sendSkyWeather():
    ...
    data = {
        ...
        "device": {"key": config.STATIONKEY, "MAC": config.STATIONMAC,},
        "utc": currentTime,
        "sensors": [
            {
            ...
            {"name": "OutsideHumidity", "value": state.OutdoorHumidity, "units": "%"},
            {
                "name": "InsideTemperature",
                "value": state.IndoorTemperature,
                "units": "C",
            },
            {"name": "InsideHumidity", "value": state.IndoorHumidity, "units": "%"},
            # {
            # 	"name":"RainInLast60Minutes",
            # 	"value": state.currentRain60Minutes,
            #                "units" : "mm/h"
            # },
            {"name": "VisibleSunlight", "value": state.SunlightVisible, "units": "lux"},
            # {
            # 		"name":"IRSunlight",
            # 	"value": state.SunlightIR,
            #                "units" : "lux"
            # },
            {"name": "UVSunlightIndex", "value": state.SunlightUVIndex, "units": ""},
            {"name": "WindSpeed", "value": state.WindSpeed, "units": "m/s"},
            {"name": "WindGust", "value": state.WindGust, "units": "m/s"},
            {"name": "WindDirection", "value": state.WindDirection, "units": "degrees"},
            {"name": "totalRain", "value": state.TotalRain, "units": "mm"},
            {
                "name": "BarometricPressure",
                "value": state.BarometricPressure,
                "units": "hPa",
            },
            {"name": "Altitude", "value": state.Altitude, "units": "m"},
            {
                "name": "SeaLevelPressure",
                "value": state.BarometricPressureSeaLevel,
                "units": "hPa",
            },
            {"name": "BarometricTrend", "value": bptrendvalue, "units": ""},
            {"name": "OutdoorAirQuality", "value": state.AQI, "units": "AQI"},
            # {
        ...
        "solarpower": [
            {"name": "BatteryVoltage", "value": state.batteryVoltage, "units": "V"},
            {"name": "BatteryCurrent", "value": state.batteryCurrent, "units": "ma"},
            {"name": "SolarVoltage", "value": state.solarVoltage, "units": "V"},
            {"name": "SolarCurrent", "value": state.solarCurrent, "units": "ma"},
            {"name": "LoadVoltage", "value": state.loadVoltage, "units": "V"},
            {"name": "LoadCurrent", "value": state.loadCurrent, "units": "ma"},
            {"name": "BatteryPower", "value": state.batteryPower, "units": "W"},
            {"name": "SolarPower", "value": state.solarPower, "units": "W"},
            {"name": "LoadPower", "value": state.loadPower, "units": "W"},
            {"name": "BatteryCharge", "value": state.batteryCharge, "units": "%"},
        ],
        "cameras": [{"name": "Sky Camera", "image": encoded_string}],
    }
