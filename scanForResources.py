...
def checkForDeviceFromIP(ip):
    ...
    myMQTTPort = 1883
    myURL = (
        "http://" + str(ip) + "/checkForID?params=" + myMQTTIP + "," + str(myMQTTPort)
    )

    completeResponse = subprocess.run(
        ["ping", "-c1", "-w 1 ", str(ip)],
        ...
        if deviceResult:
            try:
                if state.deviceStatus[str(single["id"])] == False:
                    pclogging.systemlog(
                        config.INFO, "Wireless Device ID %s Reactivated" % (myID)
                    )
            except:
                # traceback.print_exc()
                pass
            if Log:
                pclogging.systemlog(
                    config.INFO, "Wireless Device ID %s Active" % (myID)
                )
            ...
            try:
                if state.deviceStatus[str(single["id"])] == True:
                    pclogging.systemlog(
                        config.INFO, "Wireless Device ID %s has gone Inactive" % (myID)
                    )
            except:
                # traceback.print_exc()
                pass
